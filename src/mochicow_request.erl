...
-define(SAVE_BUFFER, mochiweb_request_buffer).
...
get(buffer) ->
    case erlang:get(?SAVE_BUFFER) of
        undefined ->
            put(?SAVE_BUFFER, InitialBuffer),
            InitialBuffer;
        Buf ->
            Buf
    end.
...
%%      Timeout in msec.
recv(Length, Timeout) ->
    {ok, Options} = getopts([packet, packet_size]),
    Packet = proplists:get_value(packet, Options),
    PacketSize = proplists:get_value(packet_size, Options),

    Buffer = case erlang:get(?SAVE_BUFFER) of
        undefined ->
            InitialBuffer;
        Buf ->
            put(?SAVE_RECV, true),
    ...
    case get_data(Packet, PacketSize, Length, Buffer) of
        ...
        {more, _} ->
            ...
            mochiweb_socket:setopts(Socket, [{packet, Packet},
                                             {packet_size, PacketSize}]),
            put(?SAVE_BUFFER, FinalBuffer),
            recv(Length, Timeout)
    end.
...
get_data(Packet, _PacketSize, Length, Buf)
            when Packet =:= raw; Packet =:= 0 ->
    if
        Length =:= 0 ->
            {ok, Buf, <<>>};
        byte_size(Buf) >= Length ->
            <<Data:Length/binary, Rest/binary>> = Buf,
            {ok, Data, Rest};
        true ->
            {more, Buf}
    end;
get_data(Packet, PacketSize, _Length, Buf) ->
    PacketOpts = [{packet_size, PacketSize}],
    case erlang:decode_packet(Packet, Buf, PacketOpts) of
        {more, _} ->
            {more, Buf};
        Decoded ->
            Decoded
...
cleanup() ->
    L = [?SAVE_QS, ?SAVE_PATH, ?SAVE_RECV, ?SAVE_BODY, ?SAVE_BODY_LENGTH,
         ?SAVE_POST, ?SAVE_COOKIE, ?SAVE_FORCE_CLOSE, ?SAVE_BUFFER],
    lists:foreach(fun(K) ->
                          erase(K)
                  end, L),
    ok.
...
read_chunk(0) ->
    ...
    Footers;
read_chunk(Length) ->
    case recv(2 + Length, ?IDLE_TIMEOUT) of
        <<Chunk:Length/binary, "\r\n">> ->
            Chunk;
        _ ->
            exit(normal)
